...
<div id="delete-modal-{{ $exercise->id }}" class="modal-overlay fixed inset-0 hidden items-center justify-center z-50 bg-black bg-opacity-50">
    <div class="modal-content bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
        <button onclick="closeDeleteModal({{ $exercise->id }})" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-xl">&times;</button>
        ...
        <form action="{{ route('exercises.destroy', $exercise->id) }}"  method="POST">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeDeleteModal({{ $exercise->id }})" class="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
