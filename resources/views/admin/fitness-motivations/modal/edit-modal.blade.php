...
<div id="edit-modal-{{ $motivation->id }}" class="modal-overlay fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
  <div class="modal-content bg-white rounded-lg p-6 max-w-md w-full relative">
    <!-- Close Button -->
    <button onclick="closeEditModal({{ $motivation->id }})" class="absolute top-2 right-2 text-gray-600 hover:text-orange-600 text-xl">&times;</button>
    ...
    <form method="POST" action="{{ route('fitness-motivations.update', $motivation->id) }}" class="space-y-4">
      ...
      <div>
        ...
        <textarea name="quote" required class="w-full border rounded p-2 focus:border-orange-600 focus:ring-2 focus:ring-orange-600">{{ old('quote', $motivation->quote) }}</textarea>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700">Author (optional)</label>
        <input type="text" name="author" class="w-full border rounded p-2 focus:border-orange-600 focus:ring-2 focus:ring-orange-600" value="{{ old('author', $motivation->author) }}">
      </div>

      <div class="flex gap-2 justify-end">
        <!-- Cancel button -->
        <button type="button" onclick="closeEditModal({{ $motivation->id }})" class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded">
          Cancel
        </button>
        <!-- Update button -->
