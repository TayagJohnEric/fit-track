    ...
    <form method="POST" action="{{ route('workout_templates.store') }}">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="experience_level_id" class="w-full border rounded px-4 py-2">
            @foreach($levels as $id => $name)
              <option value="{{ $id }}" {{ old('experience_level_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <select name="workout_type_id" class="w-full border rounded px-4 py-2">
            @foreach($types as $id => $name)
              <option value="{{ $id }}" {{ old('workout_type_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <input type="number" name="duration_minutes" value="{{ old('duration_minutes', 30) }}" class="w-full border rounded px-4 py-2">
        ...
        <div>
          ...
          <input type="number" name="difficulty_level" value="{{ old('difficulty_level', 1) }}" min="1" max="5" class="w-full border rounded px-4 py-2">
