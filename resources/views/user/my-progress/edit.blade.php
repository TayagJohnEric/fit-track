...
@section('content')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-gray-600">Update your weight entry for {{ $weightEntry->log_date->format('F j, Y') }}</p>
                </div>
            ...
            <!-- Weight Edit Form -->
            <form id="weightEditForm" class="max-w-md">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="date" 
                           id="log_date" 
                           name="log_date" 
                           value="{{ $weightEntry->log_date->format('Y-m-d') }}"
                           max="{{ date('Y-m-d') }}"
                           required
                ...
                <!-- BMI Preview -->
                <div id="bmiPreview" class="mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div class="mb-6 p-4 bg-blue-50 rounded-lg">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <p class="font-medium">{{ $weightEntry->log_date->format('M j, Y') }}</p>
                        </div>
                        <div>
                            <span class="text-gray-600">Original Weight:</span>
                            <p class="font-medium">{{ number_format($weightEntry->weight_kg, 1) }} kg</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const weightEntryId = {{ $weightEntry->id }};
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Calculate and display BMI preview
            ...
            // Update BMI preview when weight changes
            ...
            document.getElementById('weightEditForm').addEventListener('submit', function(e) {
                ...
                // Submit form
                fetch(`{{ route('progress.update', $weightEntry->id) }}`, {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        function deleteEntry() {
            ...
            fetch(`{{ route('progress.destroy', $weightEntry->id) }}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
